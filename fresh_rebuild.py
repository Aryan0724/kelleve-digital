...
print("=== Building fresh frontend image (no cache) ===")
...
if exit_code == 0:
    ...
    # Check it's running
    stdin3, stdout3, stderr3 = client.exec_command('docker ps --filter name=fmi_frontend --format "{{.Names}}\t{{.Status}}"')
    print("Container status:", stdout3.read().decode('utf-8'))
else:
    print("Build FAILED!")
