...
print("=== Step 2: Verify files ===")
stdin, stdout, stderr = client.exec_command('head -5 /var/www/find-my-interior/findmyinterior-frontend/src/lib/api.ts')
print(stdout.read().decode('utf-8'))
stdin, stdout, stderr = client.exec_command('grep -c "isLoggingOut" /var/www/find-my-interior/findmyinterior-frontend/src/lib/api.ts')
print("isLoggingOut occurrences:", stdout.read().decode('utf-8').strip())

print("=== Step 3: Rebuild frontend (no-cache) ===")
stdin, stdout, stderr = client.exec_command(
    'cd /var/www/find-my-interior && docker compose build --no-cache frontend 2>&1',
    timeout=600
)
