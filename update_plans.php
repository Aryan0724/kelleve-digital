...
// Bootstrap Laravel if run directly from cli, but we'll run it from tinker
...
$basePlans = [
    'starter' => [
        ...
        'monthly_wallet_credit'  => 0, // Handled as welcome bonus
    ...
    'elite' => [
        ...
        'search_ranking_boost'   => 100, // Top 3
...
foreach ($roles as $role) {
    foreach ($basePlans as $tierKey => $tierData) {
        // Only special case: business-elite is called Elite Business historically maybe? Let's check the old seeder.
        // The old seeder used "Elite Business", but "Elite" is fine too.
        $slug = "{$role}-{$tierKey}";
        $plan = SubscriptionPlan::where('slug', $slug)->first();
        
        // Let's make sure the name is formatted correctly for business elite
        if ($role === 'business' && $tierKey === 'elite') {
            $tierData['name'] = 'Elite Business';
        }

        if ($plan) {
            $plan->update($tierData);
            echo "Updated {$slug}\n";
            $updatedCount++;
        } else {
            ...
            $data = $tierData;
            $data['slug'] = $slug;
            $data['target_role_category'] = $role;
...
echo "Done! Processed {$updatedCount} plans.\n";
