...
export type Archetype = 
  ...
  | 'designer'      // interior designers, architects (existing)
  | 'worker'        // skilled trades (existing)
  | 'supplier'      // material suppliers (existing)
  | 'brand'         // brands, manufacturers (existing)
  | 'builder';      // real estate, builders (existing)
...
export interface VendorConfig {
  ...
  crmLabel: string;
  crmIcon: LucideIcon;
  uniqueTabs: { label: string; href: string; icon: LucideIcon }[];
  b2bFilters: string[]; // Slugs of B2B requirement categories relevant to them
