        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex overflow-x-auto custom-scrollbar gap-2 mb-10 pb-2">
            {['All Articles', 'Marketing', 'Sales', 'SEO & Growth', 'Tech & Trends', 'Case Studies'].map((cat, i) => (
              <button 
                key={i} 
                className={i === 0 ? 'px-6 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-md' : 'px-6 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'}
              >
                {cat}
              </button>
            ))}
