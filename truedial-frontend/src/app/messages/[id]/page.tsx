...
export default function ConversationPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="bg-background h-[calc(100vh-64px)] flex flex-col">
      ...
      <div className="bg-white border-b shrink-0">
        <div className="container mx-auto px-4 max-w-4xl h-16 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="font-bold text-navy dark:text-white truncate">{otherUser?.name || "User"}</h2>
        ...
        {conversation.project && (
          <div className="bg-primary/5 border-b shrink-0">
            <div className="container mx-auto px-4 py-3 max-w-4xl">
              <div className="flex items-start gap-3">
                <Info className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 w-full text-sm">
                  <div>
                    <span className="text-primary/70 block text-xs font-semibold uppercase tracking-wider">Project</span>
                    <span className="font-medium text-navy dark:text-white">{conversation.project.title}</span>
                  </div>
                  <div>
                    <span className="text-primary/70 block text-xs font-semibold uppercase tracking-wider">Budget</span>
                    <span className="font-medium text-navy dark:text-white">₹{conversation.project.budget_min || '0'} - ₹{conversation.project.budget_max || '0'}</span>
                  </div>
                  <div>
                    <span className="text-primary/70 block text-xs font-semibold uppercase tracking-wider">Status</span>
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-background text-foreground border border-border capitalize">
      ...
      {/* Chat Area */}
      <div id="chat-scroll-container" className="flex-1 overflow-y-auto bg-background/50 p-4">
            ...
            messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] md:max-w-[70%] rounded-2xl px-4 py-2 shadow-sm ${isMe ? 'bg-primary text-white rounded-tr-sm' : 'bg-card border border-border text-foreground rounded-tl-sm'}`}>
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2 items-end">
            <div className="flex-1 relative">
              <Input 
                ...
                className="w-full rounded-full bg-background border-border focus-visible:ring-primary pr-12"
              />
            </div>
            <Button 
              type="submit" 
              disabled={!newMessage.trim() || sending} 
              className="rounded-full w-10 h-10 p-0 bg-primary hover:bg-primary/90 text-white shrink-0 shadow-md"
            >
