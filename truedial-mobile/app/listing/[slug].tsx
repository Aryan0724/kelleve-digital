...
import { 
  Star, MapPin, Phone, MessageSquare, ShieldAlert, 
  ChevronLeft, Share2, Navigation, MessageCircle, 
  Ticket, Tag, Clock, Globe, ShieldCheck, Heart, 
  Play, CheckCircle2, ThumbsUp, Sparkles, Building, Mail, 
  Utensils, Wifi, Car, CreditCard, Users, Check, ChevronDown
  ...
  if (!business || !business.basicInfo) {
    return (
      <View className="flex-1 justify-center items-center p-10 bg-slate-950">
        ...
        <Text className="text-lg font-bold text-white mt-4 mb-5">Business Not Found</Text>
        <TouchableOpacity 
    ...
    <View className="flex-1 bg-white dark:bg-slate-950">
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1" contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        >
          ...
          {/* Top Bar Navigation */}
          <View style={{ paddingTop: insets.top }} className="flex-row justify-between items-center z-20">
            <TouchableOpacity 
          ...
          <View className="z-20 pb-4">
            <View className="flex-row items-center justify-between mb-2">
              <View className="flex-row items-center gap-2">
                <View className="bg-orange-500/80 px-2.5 py-0.5 rounded">
                  <Text className="text-white text-[10px] font-black uppercase tracking-wider">FEATURED</Text>
                </View>
              </View>

              <View className="bg-black/60 px-2 py-0.5 rounded-full border border-white/20">
                <Text className="text-white text-[10px] font-bold">1/25</Text>
              </View>
            </View>
            ...
            <View className="flex-row justify-between items-end mb-1">
              <View className="flex-1 pr-3">
                <Text className="text-2xl font-black text-white leading-tight mb-1">{basicInfo.title}</Text>
                <View className="flex-row items-center flex-wrap gap-1.5">
                  <View className="flex-row items-center bg-emerald-500/20 px-2 py-0.5 rounded border border-emerald-500/40">
                    ...
                    <Text className="text-emerald-400 text-[11px] font-bold">Verified</Text>
                  </View>
                  <Text className="text-slate-300 text-xs font-medium">• {basicInfo.category || "Multi Cuisine"}</Text>
                </View>
              </View>

              <View className="bg-emerald-600 px-2.5 py-1 rounded-xl items-center shadow-md">
                <View className="flex-row items-center">
                  <Text className="text-white font-black text-sm mr-1">4.5</Text>
                  <Star size={12} color="#FFF" fill="#FFF" />
                </View>
                <Text className="text-white/80 text-[9px] font-bold">1,248 Reviews</Text>
              </View>
            </View>

            <Text className="text-slate-300 text-xs font-medium mt-1">
              {basicInfo.address || `${basicInfo.city || 'Patna'}, Bihar`}
            </Text>
          </View>
        </ImageBackground>

        {/* 2. TRUEDIAL PRIVILEGE CARD BANNER */}
        <View className="mx-4 -mt-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/50 rounded-2xl p-3.5 flex-row justify-between items-center shadow-sm z-20">
          <View className="flex-1 pr-2">
            <View className="flex-row items-center gap-1.5">
              <Text className="text-base font-black text-amber-900 dark:text-amber-300">10% OFF</Text>
              <Text className="text-xs font-bold text-amber-700 dark:text-amber-400">Up to ₹500</Text>
            </View>
            <Text className="text-[11px] font-semibold text-amber-800/80 dark:text-amber-400/80">With TrueDial Privilege Card • T&C Apply</Text>
          </View>

          <TouchableOpacity className="bg-[#1E40AF] px-3.5 py-2 rounded-xl shadow-sm" onPress={() => router.push('/offers')}>
            <Text className="text-white text-xs font-black">View Card</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="px-4 py-5 flex-row justify-between bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
          ...
          {basicInfo.website ? (
            <TouchableOpacity className="items-center" onPress={() => handleAction(basicInfo.website)}>
              <View className="w-12 h-12 rounded-2xl bg-sky-100 dark:bg-sky-950/60 items-center justify-center mb-1 border border-sky-200 dark:border-sky-800">
            ...
            <TouchableOpacity className="items-center" onPress={handleStartChat} disabled={startingChat}>
              ...
              <Text className="text-[11px] font-bold text-slate-700 dark:text-slate-300">Chat</Text>
            </TouchableOpacity>
          )}
          ...
          >
            ...
            <Text className="text-[11px] font-bold text-slate-700 dark:text-slate-300">Save</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="px-4 py-3 bg-slate-50 dark:bg-slate-900 flex-row justify-between items-center border-b border-slate-200 dark:border-slate-800">
          <View className="flex-row items-center">
            ...
            <Text className="text-xs font-bold text-emerald-700 dark:text-emerald-400 mr-1">Open Now</Text>
        ...
        <View className="flex-row bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
          {(['Overview', 'Menu', 'Reviews', 'Offers'] as const).map((tab) => (
            ...
            >
              <Text className={`text-xs font-black ${activeTab === tab ? 'text-[#1E40AF]' : 'text-slate-500'}`}>
                {tab}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View className="p-4">
          ...
          {/* WHAT PEOPLE LOVE */}
          <Text className="text-sm font-black text-slate-900 dark:text-white mb-2.5">What People Love</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-6" contentContainerStyle={{ gap: 8 }}>
            <View className="flex-row items-center bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 px-3 py-1.5 rounded-full">
              ...
              <Text className="text-xs font-bold text-emerald-800 dark:text-emerald-300">Food Quality (512)</Text>
            </View>
            <View className="flex-row items-center bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800 px-3 py-1.5 rounded-full">
              ...
              <Text className="text-xs font-bold text-purple-800 dark:text-purple-300">Ambience (312)</Text>
            </View>
            <View className="flex-row items-center bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-full">
              ...
              <Text className="text-xs font-bold text-blue-800 dark:text-blue-300">Service (210)</Text>
            </View>
            <View className="flex-row items-center bg-orange-50 dark:bg-orange-950/40 border border-orange-200 dark:border-orange-800 px-3 py-1.5 rounded-full">
              ...
              <Text className="text-xs font-bold text-orange-800 dark:text-orange-300">Value for Money (156)</Text>
            </View>
          </ScrollView>

          {/* ABOUT THIS PLACE */}
          <Text className="text-sm font-black text-slate-900 dark:text-white mb-1.5">About This Place</Text>
          <Text className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed mb-6">
            {basicInfo.description || "A multi-cuisine restaurant offering a wide range of delicious dishes in a cozy & family-friendly ambience."}
          ...
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-2.5">
              <Text className="text-sm font-black text-slate-900 dark:text-white">Video Preview</Text>
              <Text className="text-xs font-bold text-[#1E40AF]">View All</Text>
            </View>
            <ImageBackground 
              source={{ uri: 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=800' }}
            ...
            >
              ...
              <View className="w-14 h-14 rounded-full bg-white/90 items-center justify-center shadow-lg">
                <Play size={24} color="#1E40AF" fill="#1E40AF" className="ml-1" />
              </View>
          ...
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-2.5">
              <Text className="text-sm font-black text-slate-900 dark:text-white">Photo Gallery</Text>
              <Text className="text-xs font-bold text-[#1E40AF]">View All</Text>
            </View>
            <View className="flex-row justify-between">
              {galleryImages.slice(0, 3).map((img, i) => (
                <Image key={i} source={{ uri: img }} className="w-[31%] h-24 rounded-xl" />
              ))}
              <ImageBackground source={{ uri: galleryImages[3] }} className="w-[31%] h-24 rounded-xl overflow-hidden justify-center items-center">
          ...
          {/* TOP OFFERS FOR YOU */}
          <View className="mb-6">
            <Text className="text-sm font-black text-slate-900 dark:text-white mb-2.5">Top Offers for You</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 12 }}>
              {[
                { title: "10% OFF", sub: "Up to ₹500", desc: "On total bill", date: "Valid till 31 Dec 2026" },
                { title: "15% OFF", sub: "Up to ₹750", desc: "On total bill", date: "Valid till 31 Dec 2026" },
                { title: "20% OFF", sub: "Up to ₹1000", desc: "On total bill", date: "Valid till 31 Dec 2026" }
              ].map((off, i) => (
                <View key={i} className="w-40 bg-orange-50/80 dark:bg-orange-950/40 border border-orange-200 dark:border-orange-900/50 rounded-2xl p-3 justify-between">
                  <View>
                    <Text className="text-base font-black text-orange-600 dark:text-orange-400">{off.title}</Text>
                    <Text className="text-xs font-bold text-slate-800 dark:text-slate-200">{off.sub}</Text>
                    <Text className="text-[11px] text-slate-500">{off.desc}</Text>
                  </View>
                  <View className="mt-3 pt-2 border-t border-orange-200/50 flex-row justify-between items-center">
                    <Text className="text-[9px] text-slate-400">{off.date}</Text>
                    <Text className="text-[9px] font-bold text-orange-600">T&C</Text>
                  </View>
                </View>
              ))}
            </ScrollView>
          ...
          <View className="mb-6 bg-slate-50 dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
            <View className="flex-row justify-between items-center mb-3">
              <Text className="text-sm font-black text-slate-900 dark:text-white">Ratings & Reviews</Text>
              <Text className="text-xs font-bold text-[#1E40AF]">View All</Text>
            </View>
            
            <View className="flex-row items-center">
              <View className="items-center mr-6">
                <Text className="text-3xl font-black text-slate-900 dark:text-white">4.5</Text>
                <View className="flex-row my-1">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Star key={s} size={12} color="#F59E0B" fill="#F59E0B" className="mr-0.5" />
                  ))}
                </View>
                <Text className="text-[10px] text-slate-500 font-bold">1,248 Reviews</Text>
              </View>
              ...
              <View className="flex-1 gap-1.5">
                {[
                  ...
                  <View key={b.star} className="flex-row items-center gap-2">
                    <Text className="text-[10px] font-bold text-slate-600 dark:text-slate-400 w-3">{b.star}★</Text>
                    <View className="flex-1 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                      <View className="h-full bg-emerald-500 rounded-full" style={{ width: b.pct as any }} />
                    </View>
                    <Text className="text-[9px] font-bold text-slate-400 w-6">{b.pct}</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          <View className="mb-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
            <Text className="text-sm font-black text-slate-900 dark:text-white mb-3">Contact Info</Text>
            <View className="gap-3">
              <TouchableOpacity className="flex-row items-center" onPress={() => handleAction(`tel:${basicInfo.phone}`)}>
                <Phone size={16} color="#16A34A" className="mr-3" />
                <Text className="text-xs font-bold text-slate-800 dark:text-slate-200">{basicInfo.phone || '+91 98765 43210'}</Text>
              </TouchableOpacity>
              <TouchableOpacity className="flex-row items-center" onPress={() => handleAction(`whatsapp://send?phone=${basicInfo.phone}`)}>
                <MessageCircle size={16} color="#10B981" className="mr-3" />
                <Text className="text-xs font-bold text-slate-800 dark:text-slate-200">{basicInfo.phone || '+91 98765 43210'}</Text>
              </TouchableOpacity>
              <TouchableOpacity className="flex-row items-center" onPress={() => handleAction(basicInfo.website || 'https://truedial.com')}>
                <Globe size={16} color="#1D4ED8" className="mr-3" />
                <Text className="text-xs font-bold text-[#1E40AF]">{basicInfo.website || 'www.yellowchilli.com'}</Text>
              </TouchableOpacity>
              <View className="flex-row items-center">
                <Mail size={16} color="#64748B" className="mr-3" />
                <Text className="text-xs font-bold text-slate-800 dark:text-slate-200">info@yellowchilli.com</Text>
              </View>
            </View>
          </View>
          ...
          {/* TIMINGS CARD */}
          <View className="mb-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
            <Text className="text-sm font-black text-slate-900 dark:text-white mb-3">Timings</Text>
            <View className="gap-2">
              <View className="flex-row justify-between items-center">
                <Text className="text-xs font-bold text-slate-600 dark:text-slate-400">Monday - Friday</Text>
                <Text className="text-xs font-bold text-slate-800 dark:text-slate-200">11:00 AM - 11:00 PM</Text>
              </View>
              <View className="flex-row justify-between items-center">
                <Text className="text-xs font-bold text-slate-600 dark:text-slate-400">Saturday - Sunday</Text>
                <Text className="text-xs font-bold text-slate-800 dark:text-slate-200">10:00 AM - 11:30 PM</Text>
              </View>
            </View>
          </View>
          ...
          {/* LOCATION MAP CARD */}
          <View className="mb-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
            <Text className="text-sm font-black text-slate-900 dark:text-white mb-1">Location</Text>
            <Text className="text-xs font-bold text-slate-800 dark:text-slate-200 mb-0.5">{basicInfo.title}</Text>
            <Text className="text-xs text-slate-500 mb-1">{basicInfo.address || 'Boring Road, Patna, Bihar - 800001'}</Text>
            <Text className="text-[11px] font-bold text-emerald-600 mb-3">2.2 km from your location</Text>

            <ImageBackground 
              source={{ uri: 'https://images.unsplash.com/photo-1526778548025-fa2f459cd5c1?q=80&w=800' }}
            ...
            <TouchableOpacity 
              ...
              onPress={() => handleAction(`https://maps.google.com/?q=${encodeURIComponent(basicInfo.address || basicInfo.city)}`)}
            >
          ...
          {/* AMENITIES GRID */}
          <View className="mb-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
            <Text className="text-sm font-black text-slate-900 dark:text-white mb-3">Amenities</Text>
            <View className="flex-row flex-wrap justify-between gap-y-3">
              {[
                ...
                <View key={i} className="w-[30%] bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl items-center border border-slate-200/60">
                  <am.icon size={18} color="#1E40AF" className="mb-1" />
                  <Text className="text-[10px] font-bold text-slate-700 dark:text-slate-300 text-center">{am.label}</Text>
                </View>
              ))}
            </View>
          </View>
          ...
          {/* HIGHLIGHTS */}
          <View className="mb-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
            <Text className="text-sm font-black text-slate-900 dark:text-white mb-3">Highlights</Text>
            <View className="gap-2.5">
              {['Live Kitchen', 'Home Delivery', 'Takeaway Available', 'Outdoor Seating', 'Birthday Party Zone'].map((h, i) => (
                <View key={i} className="flex-row items-center">
                  <Check size={14} color="#16A34A" className="mr-2" />
                  <Text className="text-xs font-bold text-slate-700 dark:text-slate-300">{h}</Text>
                </View>
              ))}
            </View>
          </View>
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-orange-50 dark:bg-slate-900 border-t border-orange-200 dark:border-slate-800 p-3.5 flex-row justify-between items-center shadow-lg">
        <View className="flex-1 pr-3">
          <View className="flex-row items-center gap-1">
            <Text className="text-base font-black text-orange-600">10% OFF</Text>
            <Text className="text-xs font-bold text-slate-800 dark:text-white">With TrueDial Privilege Card</Text>
          </View>
          <Text className="text-[10px] text-slate-500">Up to ₹500 discount on your bill</Text>
        </View>

        <TouchableOpacity 
          className="bg-[#EA580C] px-4 py-3 rounded-xl shadow-md"
          onPress={() => router.push('/offers')}
        >
          <Text className="text-white text-xs font-black">View Card & Avail Offer</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
