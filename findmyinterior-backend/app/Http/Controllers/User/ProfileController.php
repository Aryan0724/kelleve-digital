...
class ProfileController extends Controller
{
    ...
    public function updateListing(Request $request, int $id): JsonResponse
    {
        ...
        // Use badge_type field as the canonical indicator — not hardcoded plan slug names.
        // Elite badge_type = 'elite', which grants custom URL capability.
        $hasCustomUrlAccess = $user->isAdmin() || $plan?->badge_type === 'elite';

        if (!empty($data['custom_slug'])) {
            if (!$hasCustomUrlAccess) {
                return response()->json([
                    'success' => false,
    ...
    public function addGalleryImages(Request $request, int $id): JsonResponse
    {
        ...
        return DB::transaction(function () use ($request, $listing, $user, $entitlement) {
            // Lock listing row and count current images
            $maxImages = $entitlement->getLimit($user, 'max_gallery_images');
            $currentCount = ListingGallery::where('listing_id', $listing->id)
                ->lockForUpdate()
                ->count();

            $imagesToUpload = count($request->images);
            if (($currentCount + $imagesToUpload) > $maxImages) {
                return $entitlement->generateErrorResponse(
                    'max_gallery_images',
                    $maxImages,
                    $currentCount,
                    "You have reached your portfolio limit of {$maxImages} images."
                );
            }
            ...
            foreach ($request->images as $index => $image) {
                ...
                // If the frontend sends a raw Base64 string, convert it to an actual file
