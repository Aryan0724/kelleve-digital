...
export function PopupAd({ targetCity, targetCategoryId, frequencyHours = 24 }: PopupAdProps) {
  ...
  useEffect(() => {
    ...
    if (lastSeenStr) {
      ...
      if (hoursSinceLastSeen < frequencyHours) {
        return; // Skip fetching and showing if within frequency limit
      }
    }
  ...
  }, [targetCity, targetCategoryId, frequencyHours]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity duration-300">
      <div className="relative max-w-lg w-full bg-white dark:bg-card rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <button 
          onClick={handleClose}
