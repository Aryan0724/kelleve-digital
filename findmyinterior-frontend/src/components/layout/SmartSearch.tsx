  ...
  return (
    <div ref={containerRef} className="hidden lg:flex flex-1 max-w-2xl relative z-50">
      <form onSubmit={handleSearch} className="flex flex-1 items-center bg-gray-50/80 dark:bg-slate-800/80 backdrop-blur-sm border border-gray-200 dark:border-slate-700 rounded-xl p-1.5 shadow-inner transition-all duration-300 focus-within:bg-white dark:focus-within:bg-slate-800 focus-within:shadow-md focus-within:border-orange-200">
        <div className="flex items-center px-4 border-r border-gray-300 dark:border-slate-600 min-w-[130px] cursor-pointer hover:bg-gray-100/50 dark:hover:bg-slate-700/50 rounded-l-lg transition-colors py-1">
          ...
          <span className="text-sm font-semibold text-gray-700 dark:text-gray-200 ml-1">{locations[0] || "Patna"}</span>
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <ul className="py-2">
