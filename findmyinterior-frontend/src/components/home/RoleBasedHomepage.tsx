...
function LeadCard({ item, role }: { item: any; role: string }) {
  ...
  // Format budget cleanly without overflow
  const formatBudget = () => {
    if (isWorkerJob) return `₹${item.daily_rate ?? "?"}/day`;
    if (item.budget_min && item.budget_max) {
      const min = item.budget_min >= 100000 ? `₹${(item.budget_min / 100000).toFixed(1)}L` : `₹${(item.budget_min / 1000).toFixed(0)}k`;
      const max = item.budget_max >= 100000 ? `₹${(item.budget_max / 100000).toFixed(1)}L` : `₹${(item.budget_max / 1000).toFixed(0)}k`;
      return `${min} – ${max}`;
    }
    if (item.budget_min) return `From ₹${item.budget_min >= 100000 ? (item.budget_min / 100000).toFixed(1) + 'L' : (item.budget_min / 1000).toFixed(0) + 'k'}`;
  ...
  // Clean description string (strip any raw messy syntax like "wef [Details]" or JSON prefixes)
  const cleanDescription = (item.description || "")
    .replace(/^wef\s*\[Details\]\s*/i, "")
    .replace(/^\[Details\]\s*/i, "")
    .replace(/\\n/g, " ")
    .trim();

  return (
    <div className="bg-white dark:bg-slate-900/90 backdrop-blur-md rounded-2xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-orange-500/30 dark:hover:border-orange-500/30 transition-all duration-300 group flex flex-col justify-between h-full relative overflow-hidden">
      {/* Decorative ambient gradient */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-orange-500/10 via-orange-500/5 to-transparent rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none group-hover:scale-150 transition-transform duration-500"></div>

      <div className="relative z-10 space-y-3">
        {/* Top Badges & Budget Row */}
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 bg-orange-50 text-orange-600 dark:bg-orange-950/40 dark:text-orange-400 rounded-lg border border-orange-200/60 dark:border-orange-900/50">
              <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse"></span>
              {isWorkerJob ? "Job" : item.material_type ? "RFQ" : "Project Lead"}
            </span>
            {item.category?.name && (
              <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200/50 dark:border-slate-700/50 line-clamp-1">
                {item.category.name}
              </span>
            )}
          </div>
          ...
          {budgetDisplay && (
            <div className="inline-flex items-center gap-1 text-xs font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-1 rounded-lg border border-emerald-200/60 dark:border-emerald-900/50 shrink-0 shadow-sm">
        ...
        {/* Title & Description */}
        <div>
          <h4 className="font-bold text-slate-900 dark:text-white text-base md:text-lg leading-snug line-clamp-2 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">
            {item.title}
          </h4>
          {cleanDescription && (
      ...
      <div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-3 relative z-10">
        ...
        <Link href={`/requirements/${item.id}?type=${item.material_type ? 'rfq' : item.skill_required ? 'job' : 'project'}`}>
          <button className="text-xs text-white bg-slate-900 dark:bg-white dark:text-slate-900 hover:bg-orange-600 dark:hover:bg-orange-500 dark:hover:text-white px-3.5 py-2 rounded-xl font-bold flex items-center gap-1.5 transition-all shadow-sm hover:shadow-orange-500/20 active:scale-95 shrink-0">
