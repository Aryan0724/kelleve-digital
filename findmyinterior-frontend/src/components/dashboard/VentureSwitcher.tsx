...
const VENTURE_TYPES = [
  ...
  { icon: "🏷️", label: "Brands & Manufacturers", types: ["Furniture Brand", "Tile & Sanitaryware Brand", "Paint & Coatings Brand", "Lighting Brand", "Plywood & Laminate Brand", "Home Appliances Brand", "Hardware & Fittings Brand", "Other Manufacturer / Brand"] },
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="relative mt-1">
              <select required value={form.typeSlug} onChange={e => setForm({ ...form, typeSlug: e.target.value })}
                className="w-full appearance-none border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:border-orange-400 transition">
                <option value="" disabled>Select business type...</option>
                {VENTURE_TYPES.map(cat => (
                  <optgroup key={cat.label} label={`${cat.icon} ${cat.label}`}>
                    {cat.types.map(type => (
                      <option key={typeToSlug(type)} value={typeToSlug(type)}>{type}</option>
                    ))}
                  </optgroup>
                ))}
              </select>
