...
export function DesignerDashboard({ data, fetchDashboard }: { data: any, fetchDashboard: () => void }) {
  ...
  const renderSidebarButton = (id: string, icon: React.ReactNode, label: string) => (
    <button 
      ...
      className={`flex items-center p-3 md:p-4 border-b md:border-r-0 border-r dark:border-slate-800 text-left font-medium text-xs sm:text-sm md:text-base transition-colors w-full h-full ${activeTab === id ? 'bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-700 dark:text-slate-300'}`}
    >
      <div className={`mr-3 shrink-0 ${activeTab === id ? 'text-orange-600 dark:text-orange-500' : 'text-slate-400 dark:text-slate-500'}`}>
        {icon}
      </div>
      {label}
    </button>
