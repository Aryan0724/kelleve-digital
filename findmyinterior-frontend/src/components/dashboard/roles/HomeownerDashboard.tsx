              ...
              <Card>
                ...
                <CardContent>
                  {((data?.projects || []).concat(data?.rfqs || []).concat(data?.jobs || [])).length > 0 ? (
                    ...
                    <div className="text-center py-12 bg-slate-50 dark:bg-slate-800/50 border dark:border-slate-700 rounded-lg text-slate-500 dark:text-slate-400">
            ...
            {activeTab === 'dashboard' && data?.recent_blogs && data.recent_blogs.length > 0 && (
              <div className="mt-8">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white">Latest from the Blog</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {data.recent_blogs.map((blog: any) => (
                    <Link key={blog.id} href={`/blog/${blog.slug}`} className="group block">
                      <Card className="h-full overflow-hidden hover:shadow-md transition-all dark:bg-slate-900 dark:border-slate-800">
                        <div className="h-40 bg-slate-100 dark:bg-slate-800 overflow-hidden relative">
                          {blog.cover_image ? (
                            ...
                            <div className="w-full h-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-3xl">✍️</div>
                          )}
                          <Badge className="absolute top-2 left-2 bg-orange-600 border-0 text-white">{blog.category}</Badge>
                        </div>
                        <CardContent className="p-4">
                          <h4 className="font-bold text-slate-800 dark:text-slate-200 line-clamp-2 group-hover:text-orange-600">{blog.title}</h4>
                          <div className="text-xs text-slate-500 dark:text-slate-400 mt-2">{new Date(blog.published_at).toLocaleDateString()}</div>
                        </CardContent>
                      </Card>
                    </Link>
                  ))}
