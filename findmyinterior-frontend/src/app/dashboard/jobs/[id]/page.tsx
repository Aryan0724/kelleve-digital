...
import { HardHat, UserCircle, CheckCircle, MessageSquare, ArrowLeft } from "lucide-react";
...
export default function JobDetail() {
  ...
  if (!job) return <div className="p-8 text-center text-red-500">Job not found or unauthorized.</div>;

  const isClient = user?.id === job.user_id;
  const isWorker = user?.id === job.worker_id;

  const handleMarkProgress = async (status: string) => {
    try {
      await api.post(`/worker-jobs/${job.id}/progress`, { status });
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-black text-slate-900">{job.title || "Untitled Job"}</h1>
          </div>
          
          <div className="flex gap-2">
            {isWorker && job.status !== 'completed' && job.status !== 'cancelled' && (
              <select 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100">
          ...
          <div className="col-span-2 p-6">
            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">Job Details</h2>
            <div className="space-y-4">
              <div className="bg-slate-50 p-4 rounded-lg flex items-start gap-3">
                <HardHat className="w-5 h-5 text-slate-400 mt-0.5" />
                <div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-bold text-slate-900">
                      {isClient ? "You" : job.user?.name || "Client"}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="text-xs text-slate-500 font-medium mb-2">Worker</div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-bold text-slate-900">
                      {isWorker ? "You" : (job.worker_id ? "Assigned Worker" : "None yet")}
                    </div>
                  </div>
                </div>
              </div>
