...
import { MapPin, Search, Filter, Truck, Package, Factory, Boxes, ArrowRight, Clock } from "lucide-react";
        ...
        <div className="container mx-auto px-4 relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="flex-1 max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold mb-5 leading-tight text-white">
              Material Procurement (RFQs)
            </h1>
            <p className="text-white/70 max-w-2xl text-lg leading-relaxed mb-8">
              Discover bulk material requirements from verified contractors, interior designers, and builders. Submit your best quotation and secure supply contracts.
            </p>
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/10 px-4 py-2 rounded-full text-sm font-medium">
                <Factory className="w-4 h-4 text-[#ff9d5c]" /> <span>Direct from Buyers</span>
              </div>
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/10 px-4 py-2 rounded-full text-sm font-medium">
                <Truck className="w-4 h-4 text-[#ff9d5c]" /> <span>Bulk Volume Requirements</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-80 shrink-0 space-y-6">
          <Card className="p-6 border border-slate-200 shadow-xl shadow-slate-200/40 rounded-2xl bg-white sticky top-24">
            <h3 className="font-bold flex items-center gap-2 mb-6 text-slate-800 text-lg border-b border-slate-100 pb-4">
              <Filter className="w-5 h-5 text-[#E8701A]" /> Refine Requests
            </h3>
            <div className="space-y-5">
              <div>
                <label className="text-sm font-semibold mb-2 block text-slate-700">Material Category</label>
                <div className="relative">
                  <Boxes className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <Input placeholder="e.g., Cement, Paint, Timber..." className="pl-9 bg-slate-50 border-slate-200 focus:border-[#E8701A] focus:ring-[#E8701A]" />
                </div>
              </div>
              <div>
                <label className="text-sm font-semibold mb-2 block text-slate-700">Delivery Location</label>
                <div className="relative">
        ...
        <div className="flex-1 space-y-6">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-xl font-bold text-slate-800">
              {loading ? "Loading..." : `${rfqs.length} Active RFQs`}
            </h2>
          </div>
          ...
          {loading ? (
            <div className="text-center py-20 flex flex-col items-center bg-white rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-slate-500 font-medium">Fetching material requirements...</p>
            </div>
          ) : rfqs.length > 0 ? (
            rfqs.map((req: any) => (
              <div key={req.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 border border-slate-200 flex flex-col md:flex-row gap-6 overflow-hidden">
                ...
                <div className="flex-1 p-6 md:p-6 md:pl-0">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <Badge variant="outline" className={`whitespace-nowrap px-3 py-1 font-semibold ${req.status === 'open' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
                      {req.status === 'open' ? 'Accepting Quotes' : req.status}
                    </Badge>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center text-xs font-semibold text-slate-500 mb-4 gap-3">
                    {req.city && (
                      <span className="flex items-center">
                        ...
                        Delivery: {typeof req.city === 'string' ? req.city : req.city.name}
                      </span>
                    )}
                    <span className="flex items-center">
                  ...
                  <div className="flex items-center gap-3">
                     <span className="text-xs font-bold px-3 py-1.5 rounded-md bg-slate-100 text-slate-600">
                        Buyer: <span className="capitalize">{req.creator_role?.replace('_', ' ') || 'Builder'}</span>
                     </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-4 bg-slate-50 md:bg-transparent p-6 md:w-[220px] md:border-l border-slate-100 justify-center">
                  <div className="text-center md:text-right">
                     <div className="text-xs text-slate-500 font-medium mb-1">Target Budget</div>
                     <div className="font-extrabold text-slate-900 text-lg">
                     {req.budget_min && req.budget_max ? `₹${(req.budget_min/1000).toFixed(0)}k - ₹${(req.budget_max/1000).toFixed(0)}k` : (req.budget ? `₹${req.budget}` : "Best Quote Required")}
                     </div>
                  </div>
                  <Link href={`/requirements/${req.id}?type=rfq`} className="w-full">
            ...
            <div className="text-center py-24 bg-white rounded-2xl border border-slate-200 shadow-sm">
              ...
              <h3 className="text-2xl font-bold text-slate-800 mb-3">No active material requests found</h3>
              <p className="text-slate-500 max-w-md mx-auto text-lg">
                Check back later or adjust your filters to find buyers sourcing materials in your region.
              </p>
            </div>
          )}
        </div>
